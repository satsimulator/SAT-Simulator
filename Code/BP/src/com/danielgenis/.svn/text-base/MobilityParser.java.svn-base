...
public class MobilityParser {
	...
	// Simulation settings
	//public final int[] CHECK_INTERVAL_VALUES = {1,15,30,60}; //  how often do nodes scan/search for other notes. in seconds
	//public final int[] DELTA_VALUES = {30,60}; // how many seconds are considered "1 time interval"
	//public final int[] RANGE_VALUES = {15,30,45}; // scanning range in meters.
	public final int[] CHECK_INTERVAL_VALUES = {1}; //  how often do nodes scan/search for other notes. in seconds
	public final int[] DELTA_VALUES = {30}; // how many seconds are considered "1 time interval"
	public final int[] RANGE_VALUES = {45}; // scanning range in meters.
	...
	public final int[] TARGET_ID_VALUES = {7073}; // the node id which  get's "scanned/searched" for
	
	public static final boolean DETECT_ALL = true; // this is a VERY EXPENSIVE operation. 
	...
	public void start() {
		...
		for(Configuration c : conf) {
			try {
				...
				System.out.println("Error: Closing file handle failed.");
				e.printStackTrace();
			}
		}
	...
	private void computeTracking() {
		...
		// parses current second data into list
		// if end of input is reached. list will be empty. break loop
		...
		for(Configuration c : conf) {
			...
			if(currentSecond % c.getDELTA() == 0 && currentSecond != 0) { // got a full data set
				currentConf = c;
				Triangulation tmp = new MultilaterationMeanMean().multilateration(c.getData(), c.getTargetData());
