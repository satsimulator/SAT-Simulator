...
public class MultilaterationClass {
	
	
	@SuppressWarnings("unused")
	public static Triangulation multilateration(ArrayList<ArrayList<NodeInfo>> data, HashMap<Integer,NodeInfo> targetData) {
		...
		int totalDetections = countAllDetections(data);
	...
	private static NodeInfo getMiddle(NodeInfo n1, NodeInfo n2) {
		double d = getDistance(n1, n2);
		
		if(d > (MobilityParser.currentConf.getRANGE())) { // outside of reach.
			return null;  // return null to avoid mistakes. this should never happen
		} else if (d == (MobilityParser.currentConf.getRANGE()) || d < (MobilityParser.currentConf.getRANGE())) {
			// since radius of both circles are the same. the average values can be used
			
			float x = (n1.getX() + n2.getX()) / 2 ;
			float y = (n1.getY() + n2.getY()) / 2 ;
			float z = (n1.getZ() + n2.getZ()) / 2 ; // Z axis not used.
			
			int nodeId = MobilityParser.currentConf.getTARGET_NODE();
			int second = Math.round((float)((n1.getSecond() + n2.getSecond()) / 2));
			
			return new NodeInfo(nodeId,second,x,y,z); // return computed middle point
		} else { // d == 0           n1 and n2 are at the same position
			return null; // no triangulation possible
		}
	}
	...
	private static List<NodeInfo> getWitnessNodesInRange(ArrayList<ArrayList<NodeInfo>> data, HashMap<Integer,NodeInfo> targetData) {
		...
		for(List<NodeInfo> nodeList: data) {
			...
			for (NodeInfo node: nodeList) {
				...
				// node is not a scanning node skip
				if(!MobilityParser.currentConf.SCANNING_NODES.containsKey(node.getNodeId())) continue; 
