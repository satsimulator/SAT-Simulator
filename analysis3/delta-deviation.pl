...
my $folder = "data/";
my $delimiter = "\t";
my $outputFolder = "delta-deviation/";
...
sub loopThroughFiles {
	foreach my $d (@delta) {
		...
		my $outputStddevCollect = Statistics::Descriptive::Full->new();
		foreach my $r (@range) {
			foreach my $c (@checkInterval) {
				...
				foreach my $t (@targets) {
					...
					$outputMeanCollect->add_data($mean);
					$outputStddevCollect->add_data($stddev);
		...
		writeLine($outputFile,$d . "\t" . $outputMeanCollect->mean() .
				"\t" . $outputStddevCollect->mean() . "\n");
		...
		foreach my $r (@range) {
			foreach my $c (@checkInterval) {
				foreach my $t (@targets) {
					...
					writeLine($outputFile,"#delta \t mean \t stddev \n");
