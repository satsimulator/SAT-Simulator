		...
		foreach my $d (@delta) {
			foreach my $c (@checkInterval) {
				...
				foreach my $t (@targets) {
					...
					my $outputFile = $folder . "triangulation-CI-" . $c  .  "-D" . $d . "-PCT" . $percentage . "-TGT" . $t . "-summary.csv";
				...
				writeLine($outputFile,$r . "\t" . $mean->mean() . "\n");
		...
		foreach my $r (@range) {
			foreach my $c (@checkInterval) {
				foreach my $t (@targets) {
					...
					writeLine($outputFile,"#range \t mean \n");
